...
async function generateCharts() {
    ...
    try {
        ...
        addBotMessage(`✅ Generated ${data.count} charts! Ask me anything about the data or request custom charts.`);
    } catch(e) { showToast(e.message, true); hideLoader(); }
}
...
async function sendMessage() {
    ...
    showLoader('Thinking...');
    try {
        const res = await fetch(`${API}/api/chat/${state.sessionId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ query: q })
        });
        const data = await res.json();
        if (data.is_chart_response && data.new_charts && data.new_charts.length) {
            // Append new charts to existing ones
            state.chartsData.push(...data.new_charts);
            renderCharts();
...
async function loadSuggestions() {
    ...
    try {
        ...
        const data = await res.json();
        suggestionsDiv.innerHTML = data.questions.map(q => `<button onclick="window.sendSuggestion('${q.replace(/'/g,"\\'")}')">${q}</button>`).join('');
    } catch(e) {}
}
...
function openConsultModal(name, role) {
    const modal = document.getElementById('consultModal');
    if (modal) {
        modal.style.display = 'flex';
        modal.querySelector('h3').innerText = `Book a session with ${name}`;
        // optional: store name/role in hidden fields
    }
}
...
document.getElementById('consultForm')?.addEventListener('submit', (e) => {
    e.preventDefault();
    closeConsultModal();
    showToast('Request sent! You will receive an email within 24 hours.');
});
